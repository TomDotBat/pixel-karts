...
if PIXEL.Karts.Config.Sqlite then
    ...
    function PIXEL.Karts.SetPlayerData(steamid, data, callback)
        ...
        local q,err = sql.Query(string.format([[INSERT INTO %s(steamid, data) VALUES ("%s", %s) ON CONFLICT(steamid) DO UPDATE SET steamid="%s", data=%s;]], tblPlyData, steamid, data, steamid, data))
